...
export class HttpAPIService {
    ...
    sendEvent(event: Event): void {
        ...
        this.doHTTP<EventResponse>('POST', '/v1/event/track', headers, event)
            .then((data: EventResponse) => {
                ...
                if (data.triggerData) {
                    data.triggerData.occurred = event.occurred;
    ...
    getAppConfigurations(): void {
        ...
        this.doHTTP<AppConfiguration>('GET', `/v1/app/configuration/${appID}`, headers)
            .then((json) => {
                Log.log('Received app configuration');
                GAHelper.processConfiguration(json);
            })
            .catch((error) => {
                Log.error(error);
            });
